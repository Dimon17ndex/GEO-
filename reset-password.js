...
let isUpdateUnlocked = false; // Флаг доступности вкладки замены
...
function setResetMode(mode) {
    if (mode === 'update' && !isUpdateUnlocked && currentResetMode !== 'update') {
        return; // Блокируем переключение на замену, пока запрос не выполнен
    }
    ...
    if (window.supabaseClient) {
        window.supabaseClient.auth.onAuthStateChange((event) => {
            if (event === 'PASSWORD_RECOVERY') {
                ...
                const tabUpdate = document.getElementById('tab-update-btn');
                if (tabUpdate) tabUpdate.classList.remove('disabled');
...
function injectResetStyles() {
    ...
    const css = `
        ...
        /* Форма замены выровнена вправо по макету */
        #reset-form-update {
        ...
        <div id="reset-modal-overlay" class="reset-modal-overlay">
            ...
            <div class="reset-modal-container">
                ...
                <div class="reset-tabs" id="reset-tabs">
                    ...
                    <button type="button" class="reset-tab-btn disabled" id="tab-update-btn">Замена</button>
                </div>
...
function initResetEvents() {
    ...
    formRequest?.addEventListener('submit', async (e) => {
        ...
        if (error) {
            ...
            alert('Ссылка для сброса пароля отправлена! Теперь можно задать новый пароль.');
            
            // Разблокируем вкладку замены
            isUpdateUnlocked = true;
